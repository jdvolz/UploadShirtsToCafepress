...
# read the list of things to upload
    ...
    def upload_image(upload_page, image_uri='')
		...
		img = if image_uri != ''; image_uri else job.file end
        form.file_uploads.find{|f| f.name=='txtUpload1'}.file_name = img
    ...
    def enter_image_info(info_page)#, tags=[], category='')
        ...
        form.fields.name('tags').value = @job.tags.join(',')
        selectlist = form.fields.name('category')
		if @job.section.length == 0
			selectlist.value = selectlist.options[1].value
		else
			selectlist.value = @job.section#category
		end
        return @agent.submit(form)
    end
    ...
    def create_new_basic_shop#(shopID, shopName)
        ...
        form.fields.name('StoreID').value = @job.store_name[0,14]#shopID
        form.fields.name('ShopName').value = @job.store_name	#shopName
        return form.submit()
    end
	...
	def set_store_settings(page)
		...
		form.fields.name('defaultNameTextbox').value = @job.text
		#store_settings_page.fields.name('defaultDescriptionTextbox').value = ''
    ...
    def build_your_shop(create_new_shop_page_resp)        
		...
		#store_settings_page = @agent.click page.links.find{|link| link.text == 'Set store settings'}
		
		#store_settings_page.fields.name('priceSliderValue').value = '15'
	...
	def get_shop(shop_name)
		init_agent if not agent_initialized?
		store_url = 'http://www.cafepress.com/cp/members/stores/'
		store_list_page = @agent.get(store_url)
		page = @agent.click store_list_page.links.find{|link| link.text==shop_name}
		@agent.click page.links.find{|link| link.text=="Manage Products"}
		#This entire method is for side effects of setting the store in their session		
	end
    
    def add_products(products=CHOSEN_PRODUCTS)
	init_agent if not agent_initialized?                
...
class CPBuilder
	...
	def setup(job)#add_pic='', tags=[], section='', shop_name='')
		@cp = CafePress.new(job)
		@cp.login
		upload_pic_and_info job #(job.file, job.tags, job.section)
		@shop_add_resp = @cp.create_new_basic_shop#(job.shop_name[0,14], job.shop_name)
		@build_new_shop_resp = @cp.build_your_shop(@shop_add_resp)
		@cp.logout()		
	end
	
	def upload_pic_and_info(job)# add_pic='', tags=[], section='')
		if job.file != ''
			@cp.go_to_image_basket()
			@upload_page = @cp.go_to_image_upload()
			@info_page = @cp.upload_image(@upload_page, job.file)
			@info_page_resp = @cp.enter_image_info(info_page) #(@info_page, job.tags, job.section)
		end
	...
	def append_dark(job)
		...
		@cp.login
		upload_pic_and_info job #(job.file, job.tags, job.section)
		#get shop with name shop_name
		@cp.get_shop(job.store_name)
		#add_products on that shop?
		@cp.add_products CHOSEN_PRODUCTS_DARK
		@cp.logout()		
	end
...
class Job
	attr_accessor :file, :tags, :section, :text, :store_name
	def initialize(text='', f = '', tags=[], section='', store_name='')
		@file = f
		@tags = tags
		@section = section
		@text = text
...
def read_jobs(file)
	...
	File.open(file, 'r').each_line do |line|
		...
		job = Job.new(
			frags[0].strip,
			make_filename_from_text(frags[0].strip),
...
def main()
	if ARGV.length < 3 or ARGV.length > 4
		puts "The usage is as follows: \n\n ruby main.rb [your_full_path_to_cpx_file], [store_name_seed] [start_at] [dark]"
		#exit(0)
	else
		...
		dir = ARGV[0].gsub(/index(_dark)?\.cpx/, '')
		jobs.slice(start_at, jobs.length).each do |job|
			#print "#{seed+n.call.to_s}"
			job.store_name = seed+n.call.to_s
			if dark
				job.file = dir + job.file.gsub!(/\.png$/, '_dark.png')
				builder.append_dark(job)
			else
				job.file = dir + job.file
				builder.setup(job)
			end
